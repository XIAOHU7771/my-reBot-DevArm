...
| **ROS2 (Humble) & third_party integration** | [@danieldoradotalaveron-rb](https://github.com/danieldoradotalaveron-rb) | **Gamepad teleop with IK/FK and safety measures**: Only tested in simulation -  gamepad control with inverse kinematics for end-effector motion; live robot state visualization in RViz. | [reBotArmController_ROS2](https://github.com/danieldoradotalaveron-rb/reBotArmController_ROS2) |
| **URDF / rebotarm_bringup** | [@danieldoradotalaveron-rb](https://github.com/danieldoradotalaveron-rb) | **D405 eye-in-hand TF**: xacro placeholder under `end_link` in `rebotarm_bringup` — visual boxes + TF for RViz only (no driver, depth, or intrinsics). Mount tunable via launch (`d405_mount_xyz`/`rpy`); not bracket-calibrated. Teleop FK/IK uses arm-only `fixend_core` URDF; full xacro is for RSP/RViz. | [reBotArmController_ROS2](https://github.com/danieldoradotalaveron-rb/reBotArmController_ROS2) |
