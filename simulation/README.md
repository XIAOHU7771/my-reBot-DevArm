...
```text
reBot-DevArm/
├── simulation/                 # 基础 TCP 点动仿真
│   └── rebot_sim.py
├── simulation2/                # 力传感器 / 力控抓取 / 滑移对冲
│   ├── README.md
│   ├── requirements.txt
│   ├── 1.Force_Sensor_Simulation.py
│   ├── 3.adaptive_force_control_grasp.py
...
注意：在 Windows 上安装 PyBullet 时，如果提示缺少 Visual C++ 编译工具，请先安装 Microsoft C++ Build Tools，确保勾选“使用 C++ 的桌面开发”工作负载。
...
### 力传感器与力控抓取（simulation2）

详细说明见 [`simulation2/README.md`](simulation2/README.md)。

| 脚本 | 内容 |
|------|------|
| `1.Force_Sensor_Simulation.py` | 虚拟力/触觉：正压力实时打印 + CSV/曲线 |
| `3.adaptive_force_control_grasp.py` | 自适应力控（铁块 + 海绵；`--force-control` / `--no-force-control`） |
| `4.slip_compensation_test.py` | 滑移对冲：抬升后向下外力，自动加紧 |
...
python 3.adaptive_force_control_grasp.py --force-control
python 3.adaptive_force_control_grasp.py --no-force-control --only sponge
python 4.slip_compensation_test.py --compensate
```
